...
const ThreadsIcon = ({ className = "h-5 w-5" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor">
...
export default function Footer({ settings }: { settings?: SiteSettings }) {
  ...
  const schoolName = settings?.schoolName || "Brain Yield Schools";
  const nameWords = schoolName.split(" ");
  const nameFirst = nameWords.slice(0, -1).join(" ") || nameWords[0];
  const nameLast = nameWords.length > 1 ? nameWords[nameWords.length - 1] : "";
  const blurb = settings?.footerBlurb ||
  ...
  const email = settings?.email || "brainyield.schools2020@gmail.com";
  const fullAddress = settings
    ? `${settings.address}, ${settings.addressLocality}, ${settings.addressRegion}, ${settings.addressCountry}`
    : "Best One Road, Salasala, Kinondoni, Dar es Salaam, Tanzania";
  const firstOfficeHour = settings?.officeHours?.[0];
  const socials = [
    { href: settings?.youtubeUrl || "https://www.youtube.com/@brainyieldschools", label: "YouTube", Icon: Youtube },
    { href: settings?.instagramUrl || "https://www.instagram.com/brainyieldschools", label: "Instagram", Icon: Instagram },
    { href: settings?.facebookUrl || "https://www.facebook.com/brainyieldschools", label: "Facebook", Icon: Facebook },
    { href: settings?.threadsUrl || "https://www.threads.net/@brainyieldschools", label: "Threads", Icon: ThreadsIcon },
    { href: settings?.tiktokUrl || "https://www.tiktok.com/@brainyieldschools", label: "TikTok", Icon: TikTokIcon },
  ].filter((s) => s.href);
        ...
        <div className="grid grid-cols-1 gap-12 py-20 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <span className="text-xl font-bold">{nameFirst}</span>
                {nameLast && <span className="block text-xs text-white/70">{nameLast}</span>}
              </div>
            </div>
            ...
            <ul className="space-y-4 text-sm text-white/70">
              ...
              {firstOfficeHour && (
                <li className="flex items-start gap-3">
                  ...
                  <span>{firstOfficeHour.label}: {firstOfficeHour.hours}</span>
                </li>
              )}
            </ul>
          </div>
